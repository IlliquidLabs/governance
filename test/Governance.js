...
describe('Governance', function () {
    before(async function () {
        ...
        this.Weth = await ethers.getContractFactory("Honey");
        this.AltCoin = await ethers.getContractFactory("Honey");

        this.Factory = await ethers.getContractFactory("HoneyFactory");

        this.MockPairWeth = await ethers.getContractFactory("contracts/MockContract.sol:MockContract");
    });
    ...
    beforeEach(async function () {
        ...
        this.honeyHandle2 = await this.honey.connect(this.addr2);
        this.honeyHandle3 = await this.honey.connect(this.addr3);
        ...
        // GovernorAlpha
        this.governor = await this.Governor.deploy(this.timelock.address, this.honey.address, OWNER_ADDRESS);
        ...
        this.lpManager = await this.LpManager.deploy(this.wweth.address, this.honey.address,
            this.treasury.address);
        await this.lpManager.deployed();

        // Mock Weth Pair
        this.mockPairWeth = await this.MockPairWeth.deploy();
        await this.mockPairWeth.deployed();
        // Setup mocks
        await this.mockPairWeth.givenMethodReturnAddress(token0, this.wweth.address);
        await this.mockPairWeth.givenMethodReturnAddress(token1, this.altCoin.address);
    ...
    describe("Timelock Proposals", function () {
        ...
        it('Add whitelisted Pool', async function () {
            ...
            expect(await this.lpManager.isWhitelisted(this.mockPairWeth.address)).to.be.false;
            await expect(this.lpManager.addWhitelistedPool(this.mockPairWeth.address,1)).to.be.revertedWith('Ownable: caller is not the owner');

            const target = this.lpManager.address;
            const value = 0;
            const sig = 'addWhitelistedPool(address,uint256)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address','uint256'], [this.mockPairWeth.address, 1]);
            ...
            expect(await this.lpManager.isWhitelisted(this.mockPairWeth.address)).to.be.true;
        });
        ...
        it('Remove Whitelisted Pool', async function () {
            ...
            // Whitelist the pool
            await this.lpManager.addWhitelistedPool(this.mockPairWeth.address, 1);
            expect(await this.lpManager.isWhitelisted(this.mockPairWeth.address)).to.be.true;

            // Set LpManager owner
            await this.lpManager.transferOwnership(this.timelock.address);
            await expect(this.lpManager.removeWhitelistedPool(this.mockPairWeth.address)).to.be.revertedWith('Ownable: caller is not the owner');

            const target = this.lpManager.address;
            const value = 0;
            const sig = 'removeWhitelistedPool(address)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address'], [this.mockPairWeth.address]);
        ...
        it('Spend From Community Treasury', async function () {
            ...
            expect(await this.honey.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(0);
            ...
            await ethers.provider.send("evm_setNextBlockTimestamp", [updateTime]);
            await this.timelock.executeTransaction(target, value, sig, callData, startTime + DELAY);
            expect(await this.honey.balanceOf(UNPRIVILEGED_ADDRESS)).to.equal(transferAmount);
    ...
    describe("Governor Proposals", function () {
        it('Change Admin', async function () {
            ...
            // Transfer HONEY
            await this.honey.transfer(this.addr2.address, this.proposalThreshold);
            await this.honey.transfer(this.addr3.address, this.voteThreshold);

            // Delegate HONEY so addresses can vote, must delegate to themselves
            await this.honeyHandle2.delegate(this.addr2.address);
            await this.honeyHandle3.delegate(this.addr3.address);
            await this.honey.delegate(OWNER_ADDRESS);
        ...
        it('Add whitelisted Pool', async function () {
            ...
            const value = 0;
            const sig = 'addWhitelistedPool(address,uint256)';
            const callData = ethers.utils.defaultAbiCoder.encode(['address', 'uint256'], [this.mockPairWeth.address, 5]);
        ...
        it('Remove Whitelisted Pool', async function () {
            ...
            // Whitelist the pool
            await this.lpManager.addWhitelistedPool(this.mockPairWeth.address, 5);
            expect(await this.lpManager.isWhitelisted(this.mockPairWeth.address)).to.be.true;
