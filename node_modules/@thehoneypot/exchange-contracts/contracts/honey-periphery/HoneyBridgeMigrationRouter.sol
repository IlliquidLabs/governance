...
import "../honey-lib/libraries/TransferHelper.sol";
import "./interfaces/IBridgeToken.sol";
import "./libraries/Roles.sol";
import "./libraries/HoneyLibrary.sol";

contract HoneyBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'HoneyBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'HoneyBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "HoneyBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "HoneyBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "HoneyBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "HoneyBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IHoneyERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IHoneyPair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = HoneyLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
            quote0 = HoneyLibrary.quote(amountIn1, reserve1, reserve0);
        }
        ...
        liquidityAmount = IHoneyPair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IHoneyPair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        require(pairA != address(0), "HoneyBridgeMigrationRouter: liquidityPairFrom address 0");
        require(pairA != address(0), "HoneyBridgeMigrationRouter: liquidityPairTo address 0");
        require(pairA != pairB, "HoneyBridgeMigrationRouter: Cant convert to the same liquidity pairs");
        require(
            IHoneyPair(pairA).token0() == IHoneyPair(pairB).token0() ||
            IHoneyPair(pairA).token0() == IHoneyPair(pairB).token1() ||
            IHoneyPair(pairA).token1() == IHoneyPair(pairB).token0() ||
            IHoneyPair(pairA).token1() == IHoneyPair(pairB).token1(),
            "HoneyBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "HoneyBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "HoneyBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IHoneyPair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IHoneyPair(liquidityPairFrom).token0();
        if (
            IHoneyPair(liquidityPairFrom).token0() == IHoneyPair(liquidityPairTo).token0() ||
            IHoneyPair(liquidityPairFrom).token0() == IHoneyPair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IHoneyPair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "HoneyBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IHoneyPair(liquidityPairTo).token0() ||
            newTokenAddress == IHoneyPair(liquidityPairTo).token1(),
            "HoneyBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IHoneyPair(liquidityPairFrom).token0()) {
                amountToSwap = amountTokenB;
            }
            _migrateToken(tokenToMigrate, amountToSwap);
        }
        if (IHoneyPair(liquidityPairFrom).token0() != IHoneyPair(liquidityPairTo).token0() &&
            IHoneyPair(liquidityPairFrom).token1() != IHoneyPair(liquidityPairTo).token1()
        ) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IHoneyPair(liquidityPairTo).token0(), IHoneyPair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IHoneyPair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IHoneyPair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IHoneyPair(pairAddress).getReserves();
        uint totalSupply = IHoneyPair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        require(liquidityPairFrom != address(0), "HoneyBridgeMigrationRouter: liquidityPairFrom address 0 not supported");
        require(liquidityPairTo != address(0), "HoneyBridgeMigrationRouter: liquidityPairTo address 0 not supported");
        ...
        (uint112 reserve0, uint112 reserve1,) = IHoneyPair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = HoneyLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
